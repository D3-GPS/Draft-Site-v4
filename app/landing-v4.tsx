...
type MasterCategory = { id: string; label: string; heroSubline: string; primaryPrograms: Program[]; supportPrograms?: Program[] };
type Recommendation = {
  masterId: string;
...
const LEVEL_META: Record<LevelKey, { title: string; priceLabel: string; delivery: string }> = {
  starter: { title: "Level 1-2", priceLabel: "$97 - $297", delivery: "Self-directed or light guidance with clean structure" },
  growth: { title: "Level 3-4", priceLabel: "Application required", delivery: "Guided implementation with stronger accountability" },
  accelerator: { title: "Level 5", priceLabel: "Application required", delivery: "Higher-touch support inside a stronger room" },
  elite: { title: "Level 6", priceLabel: "Book a call", delivery: "Private-path support, strategic leverage, and direct access" },
};

function getLevelMeta(level: LevelKey) { return LEVEL_META[level]; }
function titleCaseFromId(id: string) { return id.split("-").map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" "); }
function getCategoryByGoal(goal: GoalValue) { return goal === "ai-tech" ? "ai" : goal; }
...
function buildRecommendationFromPlan(params: { goal: GoalValue; situation: SituationValue; challenge: ChallengeValue; experience: ExperienceValue; commitment: CommitmentValue; }): Recommendation {
  ...
  return {
    masterId, activeProgramId, level, cta,
    ...
    delivery: levelMeta.delivery, priceLabel: levelMeta.priceLabel, unlocks: learnMore.unlocks,
    learnMoreTitle: learnMore.title, learnMoreBody: learnMore.body, bullets: learnMore.bullets, note: learnMore.note,
  };
}
...
function stepTitle(step: number) {
  switch (step) {
    case 0: return "What are you trying to improve?";
    case 1: return "Where are you right now?";
    case 2: return "What is the biggest issue?";
    case 3: return "How experienced are you?";
    default: return "How do you want to move?";
  }
}
...
.page { min-height: 100vh; background: #fff; color: #111; }
...
.h3 { font-size: 24px; line-height: 1.2; font-weight: 700; margin: 0; }
...
.grid-2 { display: grid; gap: 32px; grid-template-columns: repeat(2, minmax(0,1fr)); }
...
.btn.secondary { border-color: #d1d5db; background: #fff; color: #111; }
.card { border: 1px solid #e5e7eb; border-radius: 32px; background: #fff; }
.card.soft { background: #fafafa; border-radius: 24px; }
...
@media (max-width: 900px) { .grid-2,.grid-3,.grid-4 { grid-template-columns: 1fr; } .hero { padding: 72px 24px; } }
`; }

function chipStyle(active: boolean): React.CSSProperties {
  return { border: active ? "1px solid #111" : "1px solid #d1d5db", background: active ? "#111" : "#fff", color: active ? "#fff" : "#111", borderRadius: 9999, padding: "10px 16px", fontSize: 14, cursor: "pointer" };
}
...
export default function LandingPageV4() {
  ...
  const [activeRecommendation, setActiveRecommendation] = useState<Recommendation>(() => buildRecommendationFromPlan({ goal: "wealth", situation: "early", challenge: "money-clarity", experience: "beginner", commitment: "guided" }));
  ...
  useEffect(() => {
    ...
    setSelectedPrimaryId((prev) => (prev && selectedMaster.primaryPrograms.some((p) => p.id === prev) ? prev : nextPrimary?.id ?? ""));
  }, [selectedMaster]);
  ...
  const explorerRecommendation = useMemo(() => {
    ...
    if (desiredLevel === base.level && base.masterId === selectedMasterId && base.activeProgramId === activeProgramId) return activeRecommendation;
    const levelMeta = getLevelMeta(desiredLevel);
    const learnMore = buildLearnMore(selectedMasterId, activeProgramId, desiredLevel);
    return { ...activeRecommendation, masterId: selectedMasterId, activeProgramId, level: desiredLevel, cta: getCtaFromLevel(desiredLevel), summary: `${titleCaseFromId(activeProgramId)} - ${levelMeta.title}`, delivery: levelMeta.delivery, priceLabel: levelMeta.priceLabel, unlocks: learnMore.unlocks, learnMoreTitle: learnMore.title, learnMoreBody: learnMore.body, bullets: learnMore.bullets, note: learnMore.note };
  ...
  const progressValue = useMemo(() => ({ starter: 25, growth: 55, accelerator: 80, elite: 100 }[explorerRecommendation.level]), [explorerRecommendation.level]);
  ...
  const openPlanModal = () => { setCurrentStep(0); setPlanModalOpen(true); };
  const applyPlanRecommendation = () => {
    ...
    setTimeout(() => { recommendationRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }); }, 50);
  };

  const renderStepOptions = () => {
    const makeButton = (value: string, label: string, active: boolean, onClick: () => void) => (
      <button key={value} type="button" onClick={onClick} className={`option-btn ${active ? "active" : ""}`}>{label}</button>
    );
    if (currentStep === 0) return [
      makeButton("wealth","Build wealth",goal==="wealth",()=>{setGoal("wealth");setCurrentStep(1);} ),
      makeButton("business","Grow a business",goal==="business",()=>{setGoal("business");setCurrentStep(1);} ),
      makeButton("marketing","Get more customers",goal==="marketing",()=>{setGoal("marketing");setCurrentStep(1);} ),
      makeButton("sales","Improve sales performance",goal==="sales",()=>{setGoal("sales");setCurrentStep(1);} ),
      makeButton("life-design","Create more freedom in my life",goal==="life-design",()=>{setGoal("life-design");setCurrentStep(1);} ),
      makeButton("ai-tech","Use AI / tech better",goal==="ai-tech",()=>{setGoal("ai-tech");setCurrentStep(1);} ),
    ];
    if (currentStep === 1) return [
      makeButton("early","Early stage / no structure",situation==="early",()=>{setSituation("early");setCurrentStep(2);} ),
      makeButton("stable","Stable income",situation==="stable",()=>{setSituation("stable");setCurrentStep(2);} ),
      makeButton("owner","Business owner",situation==="owner",()=>{setSituation("owner");setCurrentStep(2);} ),
      makeButton("investing","Already investing",situation==="investing",()=>{setSituation("investing");setCurrentStep(2);} ),
      makeButton("scaling","Scaling",situation==="scaling",()=>{setSituation("scaling");setCurrentStep(2);} ),
    ];
    if (currentStep === 2) return [
      makeButton("money-clarity","I do not know where my money is going",challenge==="money-clarity",()=>{setChallenge("money-clarity");setCurrentStep(3);} ),
      makeButton("income","I do not have enough income",challenge==="income",()=>{setChallenge("income");setCurrentStep(3);} ),
      makeButton("system","I do not have a system",challenge==="system",()=>{setChallenge("system");setCurrentStep(3);} ),
      makeButton("opportunities","I do not have enough opportunities",challenge==="opportunities",()=>{setChallenge("opportunities");setCurrentStep(3);} ),
      makeButton("stuck","I feel stuck / not progressing",challenge==="stuck",()=>{setChallenge("stuck");setCurrentStep(3);} ),
    ];
    if (currentStep === 3) return [
      makeButton("beginner","Beginner",experience==="beginner",()=>{setExperience("beginner");setCurrentStep(4);} ),
      makeButton("some","Some experience",experience==="some",()=>{setExperience("some");setCurrentStep(4);} ),
      makeButton("advanced","Advanced",experience==="advanced",()=>{setExperience("advanced");setCurrentStep(4);} ),
    ];
    return [
      makeButton("self-directed","Self-directed",commitment==="self-directed",()=>setCommitment("self-directed") ),
      makeButton("guided","Guided program",commitment==="guided",()=>setCommitment("guided") ),
      makeButton("high-level","High-level / fast results",commitment==="high-level",()=>setCommitment("high-level") ),
    ];
  };
      ...
      <div className="page">
        <section className="section">
          <div className="container-narrow hero">
            ...
            <h1 className="h1">Stop guessing your growth.<br />Start becoming the kind of operator who runs inside a system.</h1>
            <p className="sub">D3 is built for people who know more effort is not the answer. It gives you a structured path, 12-week execution cycles, and the right environment to grow into higher levels of capability, performance, and opportunity.</p>
            <div className="grid-3 mt-8">
              {["12-week execution cycles", "Clear progression by level", "Stronger rooms as you grow"].map((item) => <div key={item} className="card soft card-pad-md small">{item}</div>)}
            </div>
            <div className="button-row">
              <button className="btn" onClick={openPlanModal}>Get My Plan</button>
              <a href="#progression" className="btn secondary">Explore Systems</a>
            </div>
          </div>
        </section>
        ...
        <section className="section">
          <div className="container section-pad" style={{ textAlign: "center" }}>
            ...
            <div className="grid-4 mt-8">
              {["It is structured around execution, not inspiration.","It gives people a path, not just information.","It increases sophistication over time.","It upgrades both capability and environment."].map((item) => <div key={item} className="card soft card-pad-md small">{item}</div>)}
            </div>
          </div>
        </section>
        ...
        <section id="progression" className="section">
          <div className="container section-pad">
            <div className="mb-12" style={{ maxWidth: 760 }}>
              ...
              <p className="sub" style={{ margin: "12px 0 0", maxWidth: "none" }}>The engine stays the same. What changes is the depth of thinking, the quality of discussion, and the environment you grow inside.</p>
            </div>
            <div className="grid-3">
              {levels.map((lvl, idx) => {
                const isActive = (explorerRecommendation.level === "starter" && idx <= 1) || (explorerRecommendation.level === "growth" && idx >= 2 && idx <= 3) || (explorerRecommendation.level === "accelerator" && idx === 4) || (explorerRecommendation.level === "elite" && idx === 5);
                return <div key={lvl.title} className="card" style={{ background: isActive ? "#fafafa" : "#fff", borderColor: isActive ? "#111" : "#e5e7eb" }}><div className="card-pad-md"><div className="tiny muted">{lvl.label}</div><div className="h3 mt-3">{lvl.title}</div><div className="mt-3" style={{ fontSize: 16, lineHeight: 1.5 }}>{lvl.desc}</div><div className="mt-4 small muted">{lvl.sub}</div></div></div>;
              })}
            </div>
          </div>
        </section>
        ...
        <section className="section">
          <div className="container-narrow section-pad" style={{ textAlign: "center" }}>
            ...
            <div className="mt-8"><button className="btn" onClick={openPlanModal}>Get My Plan</button></div>
          </div>
        </section>
          ...
          <div className="container section-pad">
            <div className="grid-2">
              <div>
                ...
                <div className="h3 mt-4">{activeRecommendation.summary}</div>
                <p className="sub" style={{ margin: "16px 0 0", maxWidth: "none" }}>{activeRecommendation.outcome}</p>
                <div className="card soft card-pad-md mt-6"><div className="small muted">Why this fits</div><div className="mt-3 small">{activeRecommendation.whyFits}</div></div>
                <div className="progress-wrap">
                  <div className="progress-bar"><div className="progress-fill" style={{ width: `${progressValue}%` }} /></div>
                  <div className="tiny muted" style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><span>Starter</span><span>Growth</span><span>Accelerator</span><span>Elite</span></div>
                </div>
                <div className="button-row" style={{ justifyContent: "flex-start" }}>
                  <a href={routeHref} className="btn">{routeLabel}</a>
                  <button className="btn secondary" onClick={() => setAdvancedModalOpen(true)}>Refine My Plan</button>
                </div>
              </div>
              <div className="card"><div className="card-pad-lg"><div className="small muted">Learn more</div><div className="h3 mt-3">{explorerRecommendation.learnMoreTitle}</div><div className="mt-4 small muted">{explorerRecommendation.learnMoreBody}</div><div className="mt-6">{explorerRecommendation.bullets.map((item) => <div key={item} className="small" style={{ display: "flex", gap: 8 }}><span>•</span><span>{item}</span></div>)}</div>{explorerRecommendation.note ? <div className="tiny muted mt-4">{explorerRecommendation.note}</div> : null}</div></div>
            </div>
          </div>
        </section>
        ...
        <section className="section">
          <div className="container section-pad">
            ...
            <div className="inline-list mt-8">{MASTER_CATEGORIES.map((item) => <button key={item.id} style={chipStyle(selectedMasterId === item.id)} onClick={() => { setSelectedMasterId(item.id); setSelectedSupportId(""); setSelectedPrimaryId(item.primaryPrograms[0]?.id ?? ""); }}>{item.label}</button>)}</div>
            <div className="card soft card-pad-lg mt-8">
              <div>
                <div className="small" style={{ fontWeight: 600, marginBottom: 12 }}>Core program</div>
                <div className="inline-list">{selectedMaster.primaryPrograms.map((program) => <button key={program.id} style={chipStyle(selectedPrimaryId === program.id && !selectedSupportId)} onClick={() => { setSelectedPrimaryId(program.id); setSelectedSupportId(""); }}>{program.label}</button>)}</div>
              </div>
              <div className="mt-6">
                <div className="small" style={{ fontWeight: 600, marginBottom: 12 }}>Support program</div>
                <div className="inline-list">{selectedMaster.supportPrograms?.map((program) => <button key={program.id} style={chipStyle(selectedSupportId === program.id)} onClick={() => { setSelectedSupportId(program.id); setSelectedPrimaryId(""); }}>{program.label}</button>)}</div>
              </div>
              <div className="mt-6" style={{ maxWidth: 280 }}>
                <label className="small" style={{ fontWeight: 600, display: "block", marginBottom: 8 }}>Recommended level</label>
                <select className="select" value={levelOverride || activeRecommendation.level} onChange={(e) => setLevelOverride(e.target.value as LevelKey)}>
                  <option value="starter">Level 1-2</option><option value="growth">Level 3-4</option><option value="accelerator">Level 5</option><option value="elite">Level 6</option>
                </select>
              </div>
              <div className="button-row" style={{ justifyContent: "flex-start", marginTop: 24 }}>
                <a href={routeHref} className="btn">{routeLabel}</a>
                <button className="btn secondary" onClick={() => setAdvancedModalOpen(true)}>Advanced Adjustments</button>
              </div>
            </div>
          </div>
        </section>

        {planModalOpen ? <div className="modal-backdrop" onClick={() => setPlanModalOpen(false)}><div className="modal modal-sm" onClick={(e) => e.stopPropagation()}><div className="modal-header"><div><div className="h3">Get My Plan</div><div className="small muted mt-3">This is a personalized roadmap generator, not a quiz. We will identify the right starting point and next step.</div><div className="tiny muted mt-3">Step {currentStep + 1} of 5</div></div><button className="close-btn" onClick={() => setPlanModalOpen(false)}>X</button></div><div className="modal-body"><div className="progress-bar"><div className="progress-fill" style={{ width: `${((currentStep + 1) / 5) * 100}%` }} /></div><div className="mt-6"><div className="h3">{stepTitle(currentStep)}</div><div className="small muted mt-3">{currentStep === 0 && "Choose the area where the real improvement matters most right now."}{currentStep === 1 && "Be honest about your current situation so the recommendation stays accurate."}{currentStep === 2 && "Pick the issue that is actually slowing you down the most."}{currentStep === 3 && "Answer based on your real experience, not your ambition."}{currentStep === 4 && "This helps us decide the right pace, support, and next step."}</div></div><div className="option-list mt-6">{renderStepOptions()}</div><div className="button-row" style={{ justifyContent: "space-between", marginTop: 24 }}><button className="btn secondary" onClick={() => setCurrentStep((s) => Math.max(0, s - 1))} disabled={currentStep === 0}>Back</button>{currentStep < 4 ? <button className="btn" onClick={() => setCurrentStep((s) => Math.min(4, s + 1))}>Continue</button> : <button className="btn" onClick={applyPlanRecommendation}>See My Plan</button>}</div></div></div></div> : null}

        {advancedModalOpen ? <div className="modal-backdrop" onClick={() => setAdvancedModalOpen(false)}><div className="modal modal-lg" onClick={(e) => e.stopPropagation()}><div className="modal-header"><div><div className="h3">Advanced Adjustments</div><div className="small muted mt-3">Use this if you already know how you want to refine the recommendation.</div></div><button className="close-btn" onClick={() => setAdvancedModalOpen(false)}>X</button></div><div className="modal-body"><div className="card soft card-pad-md small">This is the power-user view. It is here if you want tighter control over the plan.</div><div className="grid-2 mt-6"><div><label className="small" style={{ fontWeight: 600, display: "block", marginBottom: 8 }}>Category</label><select className="select" value={selectedMasterId} onChange={(e) => setSelectedMasterId(e.target.value)}>{MASTER_CATEGORIES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div><div><label className="small" style={{ fontWeight: 600, display: "block", marginBottom: 8 }}>Level</label><select className="select" value={levelOverride || activeRecommendation.level} onChange={(e) => setLevelOverride(e.target.value as LevelKey)}><option value="starter">Level 1-2</option><option value="growth">Level 3-4</option><option value="accelerator">Level 5</option><option value="elite">Level 6</option></select></div></div><div className="card mt-6"><div className="card-pad-md"><div className="small muted">Preview</div><div className="h3 mt-3">{explorerRecommendation.summary}</div><div className="mt-3 small">{explorerRecommendation.learnMoreBody}</div></div></div><div className="button-row" style={{ justifyContent: "flex-start" }}><button className="btn" onClick={() => setAdvancedModalOpen(false)}>Use These Changes</button><a href={routeHref} className="btn">{routeLabel}</a></div></div></div></div> : null}
      </div>
    </>
  );
}
